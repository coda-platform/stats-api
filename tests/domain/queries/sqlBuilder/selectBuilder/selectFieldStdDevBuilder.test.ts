...
describe('selectFieldStdDevBuilder tests', () => {
    ...
    it('with array field, gets json field array formatted as field with fields path . replaced with _ and subquery name', () => {
        ...
        // ACT
        const result = selectFieldStddevBuilder.build(field, fieldTypes, observationSelector);

        // ASSERT
        expect(result).toEqual("STDDEV(jsonb_array_elements(resource->'code'->'coding')->>'code') AS stddev");
    });
    ...
    it('gets age field from calculated fields', () => {
        ...
        // ASSERT
        expect(result).toEqual("STDDEV(CASE WHEN resource->'deceased'->>'dateTime' IS NULL THEN CASE WHEN length(resource->>'birthDate') < 7 THEN null WHEN length(resource->>'birthDate') = 7 THEN extract(year from AGE(date(resource->>'birthDate' || '-01'))) ELSE extract(year from AGE(date(resource->>'birthDate')))END ELSE CASE WHEN length(resource->>'birthDate') < 7 THEN null WHEN length(resource->>'birthDate') = 7 THEN extract(year from AGE(date(resource->'deceased'->>'dateTime'), date(resource->>'birthDate' || '-01'))) ELSE extract(year from AGE(date(resource->'deceased'->>'dateTime'), date(resource->>'birthDate'))) END END) AS stddev");
    });
});